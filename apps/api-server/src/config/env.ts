...
const envSchema = z.object({
  ...
  QUOTE_MEMORY_CACHE_TTL_MS: z.string().transform(Number).default("15000"),
  QUOTE_NULL_CACHE_TTL_MS: z.string().transform(Number).default("30000"),
  ACTIVE_SYMBOL_CACHE_TTL_MS: z.string().transform(Number).default("60000"),
  QUOTE_REFRESH_FAILURE_COOLDOWN_MS: z.string().transform(Number).default("60000"),
  ALERT_CANDIDATE_CACHE_TTL_MS: z.string().transform(Number).default("60000"),
